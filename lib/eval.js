...
function eval(exp, env) {
  var message;
  debuglog('eval', JSON.stringify(exp));
  ...
  if (is.let(exp))            return eval(__evaled__(Let.toApplication(exp), 'let'), env);
  ...
  if (is.and(exp))            return eval(__evaled__(And.toIf(exp) , 'and'), env);
  if (is.application(exp)) {
    return __evaled__(App.apply(
...
var Let = {
  bindings: S.cadr,
  boundNames: function(exp) {
    return S.map(Let.bindings(exp), S.car);
  },
  boundValues: function(exp) {
    return S.map(Let.bindings(exp), S.cadr);
  },
  body: S.cddr,
  toApplication: function(exp) {
    var params = Let.boundNames(exp),
        values = Let.boundValues(exp),
        lambda = Lambda.make(params, Let.body(exp));

    return App.make(lambda, values);
  }
};
...
var And = {
  clauses: S.cdr,
  toIf: function(exp) {
    return And.expand(And.clauses(exp));
  },
  expand: function(clauses) {
    var first = S.car(clauses),
        rest  = S.cdr(clauses),
        isLast = S.isNull(rest);

    if (isLast) return first;
...
var Proc = {
  ...
  isCompound: function(proc) {
    return S.car(proc) === 'procedure';
  },
  params: S.cadr,
  body: S.caddr,
  env: S.cadddr,
...
var App = {
  ...
  apply: function(proc, operandValues) {
    var baseEnv, newEnv, params;
    if (Proc.Primitive.isPrimitive(proc)) return App.applyPrimitive(proc, operandValues);
    if (Proc.isCompound(proc)) {
      baseEnv = Proc.env(proc);
      params = Proc.params(proc).toPrimitiveList();
      operandValues = operandValues.toPrimitiveList();
      newEnv = Env.extend(baseEnv, params, operandValues);
      return Seq.eval(Proc.body(proc), newEnv);
    }

    Util.error("Unknown procedure type -- APPLY", proc);
  },
  make: function(proc, args) {
    return S.cons(proc, args);
  }
};
