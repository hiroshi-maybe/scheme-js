...
var Primitives = {
  ...
  '<': function sub() {
    var args = Util.toArray(arguments),
        head = args[0],
        lessThanHead = lessThan.bind(null, head);

    return args.slice(1).every(lessThanHead);
  },
  '>': function sub() {
    var args = Util.toArray(arguments),
        head = args[0],
        lessThanHead = greaterThan.bind(null, head);

    return args.slice(1).every(greaterThanHead);
  }
};
